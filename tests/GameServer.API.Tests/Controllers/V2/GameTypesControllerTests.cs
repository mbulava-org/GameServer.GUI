...
public class GameTypesControllerTests
{
    ...
    public async Task Export_WhenGameTypeExists_ShouldReturnOk()
    {
        ...
        var okResult = Assert.IsType<OkObjectResult>(result.Result);
        var payload = Assert.IsType<GameServer.API.Dtos.V2.PortableGameTypePackageDto>(okResult.Value);
    ...
    public async Task Import_WhenPackageIsValid_ShouldReturnCreatedAtAction()
    {
        ...
        var result = await controller.Import(new GameServer.API.Dtos.V2.PortableGameTypePackageDto
        {
            GameType = new GameServer.API.Dtos.V2.PortableGameTypeDto
            {
                Key = "minecraft",
                DisplayName = "Minecraft",
                Type = "docker",
                CurrentRevisionVersionTag = "latest",
                Revisions = [ new GameServer.API.Dtos.V2.PortableGameTypeRevisionDto { VersionTag = "latest", ImageReference = "itzg/minecraft-server", Ports = [ new GameServer.API.Dtos.V2.PortableGameTypePortDto { ContainerPort = 25565, Protocol = "tcp", AdvertisedPort = true, DisplayOrder = 0 } ] } ]
            }
        });

        var createdResult = Assert.IsType<CreatedAtActionResult>(result.Result);
        var payload = Assert.IsType<GameServer.API.Dtos.V2.GameTypeDetailDto>(createdResult.Value);
    ...
    public async Task ScanTag_WhenRequestDoesNotRequireSavedGameType_ShouldReturnOk()
    {
        ...
        var result = await controller.ScanTag(new GameServer.API.Dtos.V2.DetectGameTypeSetupRequestDto
        {
            ImageReference = "itzg/minecraft-server",
            VersionTag = "latest"
        });
        ...
        var okResult = Assert.IsType<OkObjectResult>(result.Result);
        var payload = Assert.IsType<GameServer.API.Dtos.V2.GameTypeSetupDetectionResultDto>(okResult.Value);
