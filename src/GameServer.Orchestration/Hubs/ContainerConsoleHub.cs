...
namespace GameServer.Orchestration.Hubs;
...
public class ContainerConsoleHub : Hub
{
    ...
    /// <summary>Client sends input to the container terminal.</summary>
    public Task SendInput(string sessionId, string input)
        => _sessionManager.SendInputAsync(Context.ConnectionId, input);

    /// <summary>Disconnect from terminal.</summary>
    public Task Disconnect() => _sessionManager.CloseSessionAsync(Context.ConnectionId);
