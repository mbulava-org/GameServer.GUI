...
namespace GameServer.Docker.Client.Extensions
{
    /// <summary>
    /// Extension methods for registering GameServer.Docker client services
    /// </summary>
    public static class ServiceCollectionExtensions
    {
        ...
        /// <summary>
        /// Adds all GameServer.Docker REST API clients (auto-generated from OpenAPI) to the service collection.
        /// Requires an HttpClient factory to be registered.
        /// </summary>
        /// <param name="services">Service collection</param>
        /// <param name="baseUrl">Base URL of the GameServer.Docker API (e.g., "https://your-api.com")</param>
        /// <returns>Service collection for chaining</returns>
            ...
            string baseUrl)
        {
            ...
            // Register all auto-generated API clients as transient
            // These are generated by NSwag from the OpenAPI specification

            // Note: The actual client classes are generated at build time by NSwag
            // They follow the pattern: {Controller}Api (e.g., GameServerApi, DashboardApi, etc.)
            // Each client has a constructor that accepts HttpClient

            // Register a shared HttpClient with base address

            //I'm just going to manually register a few example clients here.
            services.AddHttpClient<IDashboardApi, DashboardApi>(client =>
            {
                client.BaseAddress = new Uri(baseUrl);
                client.Timeout = TimeSpan.FromSeconds(100);
            });
            services.AddHttpClient<IGameServerApi, GameServerApi>(client =>
            {
                client.BaseAddress = new Uri(baseUrl);
                client.Timeout = TimeSpan.FromSeconds(100);
            });
            services.AddHttpClient<IGameTypeApi, GameTypeApi>(client =>
            {
                client.BaseAddress = new Uri(baseUrl);
                client.Timeout = TimeSpan.FromSeconds(100);
            });
            services.AddHttpClient<IGameTypeExtendedMetadataApi, GameTypeExtendedMetadataApi>(client =>
            {
                client.BaseAddress = new Uri(baseUrl);
                client.Timeout = TimeSpan.FromSeconds(100);
            });
        ...
        /// <summary>
        /// Adds all GameServer.Docker client services (both REST API and SignalR) to the service collection.
        ...
        /// <param name="consoleHubUrl">URL of the Console SignalR hub (e.g., "https://your-api.com/hubs/console")</param>
        ...
        /// <summary>
        /// Adds all GameServer.Docker client services with custom configuration.
        /// </summary>
        /// <param name="services">Service collection</param>
        ...
        /// <param name="consoleHubUrl">URL of the Console SignalR hub</param>
        /// <param name="configureConsoleHub">Action to configure the Console hub connection</param>
            ...
            Action<IHubConnectionBuilder>? configureResourcesHub = null)
        {
            ...
            services.AddHttpClient();
            services.AddHttpClient("GameServer.Docker.Api", client =>
            {
