...
```bash
dotnet add package GameServer.API.Client
```
...
The REST API clients are auto-generated using [NSwag](https://github.com/RicoSuter/NSwag) from the GameServer.API API's OpenAPI specification. The generation happens during build via MSBuild target.

To regenerate the clients manually:
```bash
cd src/GameServer.API.Client
dotnet build
```

The generated client code is in `GameServer.API.Client.v1.g.cs`.

### NSwag Configuration

The client generation is configured in `nswag.json`:
- Source: `../GameServer.API/GameServer.API.csproj`
- Target Framework: `net10.0`
- Output: `GameServer.API.Client.v1.g.cs`
- Namespace: `GameServer.API.Client`
- JSON Library: Newtonsoft.Json

## Requirements

- .NET 10.0 or later
- GameServer.API API endpoint
...
Development history available on [GitHub](https://github.com/mbulava-org/GameServer.API).

## Contributing

Contributions are welcome! Please open an issue or submit a pull request on [GitHub](https://github.com/mbulava-org/GameServer.API).
...
For issues and questions, please visit the [GitHub repository](https://github.com/mbulava-org/GameServer.API).
